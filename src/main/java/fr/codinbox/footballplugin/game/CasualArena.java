...
public class CasualArena implements Arena {
    ...
    public CasualArena(FootMode mode, MinecraftWorld minecraftWorld) {
    ...
    public String getPassword() {
        // No password for casual matches
        return null;
    }
    ...
    public void joinPlayer(Player player, boolean spectator) {
        if(GameState.INGAME.equals(this.state)) {
            if(getPlayerTeam(player) != null) {
                ...
                NametagEdit.getApi().setPrefix(player, getPlayerTeam(player).getQualifier().getColorCode().replace("§", "&"));
                if(!this.playerGoals.containsKey(player.getUniqueId()))
                    this.playerGoals.put(player.getUniqueId(), 0);
                if(!this.playerPasses.containsKey(player.getUniqueId()))
                    this.playerPasses.put(player.getUniqueId(), 0);
                Bukkit.getScheduler().runTaskLater(FootballPlugin.INSTANCE, () -> teleportPlayerToSpawn(player), 20*1);
                return;
            }
        }
        if(!spectator) {
            FootTeam smallestTeam = null;
            ...
            for(FootTeamQualifier qualifier : this.teams.keySet()) {
                ...
                if(smallestTeam.getPlayersUuid().size() > team.getPlayersUuid().size())
                    smallestTeam = team;
            }

            smallestTeam.getPlayersUuid().add(player.getUniqueId());
            ...
            player.setFoodLevel(20);
            player.setGameMode(GameMode.SURVIVAL);
            player.teleport(getRandomSpawn().toLocation(this.minecraftWorld.getWorld()));

            NametagEdit.getApi().setPrefix(player, getPlayerTeam(player).getQualifier().getColorCode().replace("§", "&"));

            playerExecutor.broadcastMessage(new LanguageManager.Phrase(LanguageKey.ARENA_PLAYER_JOIN)
                    .replaceVar("name", player.getName())
                    .replaceVar("current", String.valueOf(getIngamePlayers().size()))
                    .replaceVar("max", String.valueOf(this.mode.getNumberOfSpawns() * this.teams.size()))
                    .toString());

            createScoreboard(player);

            // Create BossBar for spectator player
            bossBar.createBossBar(player);

            this.playerGoals.put(player.getUniqueId(), 0);
            this.playerPasses.put(player.getUniqueId(), 0);

            if(getIngamePlayers().size() >= mode.getNumberOfSpawns()*teams.size())
    ...
    public void leavePlayer(OfflinePlayer player) {
        ...
        if(this.state.equals(GameState.ENDING) || this.state.equals(GameState.WAITING) || this.state.equals(GameState.STARTING)) {
        ...
        switch (this.state) {
            case WAITING:
            case STARTING:
                playerExecutor.broadcastMessage(new LanguageManager.Phrase(LanguageKey.ARENA_PLAYER_LEAVE)
                    .replaceVar("name", player.getName())
                    .replaceVar("current", String.valueOf(getIngamePlayers().size()))
                    .replaceVar("max", String.valueOf(this.mode.getNumberOfSpawns() * this.teams.size()))
                    .toString());
        ...
        if(player.isOnline()) {
            ...
            bossBar.removeBossBar(onlinePlayer);
    ...
    public void goal(FootTeam scoredTeam, Location location) {
        ...
        if(GameState.INGAME.equals(this.state) && lastBallHitEntry != null) {
            ...
            playerExecutor.broadcastMessage(new LanguageManager.Phrase(LanguageKey.GOAL_SCORED)
                    ...
                    .replaceVar("playername", scorePlayer.getName())
                    .toString());
                ...
                if(!lastBallPassHitEntry.getValue().equals(lastBallHitEntry.getValue())) {
                    ...
                    if(scoreTeam.equals(getPlayerTeam(passingPlayer)) && totalTravelTime <= FootConfig.MAX_ASSIST_TIME) {
                        playerExecutor.broadcastMessage(new LanguageManager.Phrase(LanguageKey.GOAL_ASSIST)
                                .replaceVar("playername", passingPlayer.getName())
                                .toString());
            ...
            public void run() {
                if(timer <= 3) {
                    ...
                    if(timer == 0) {
                        ...
                        centerAndCountDownEnd(gamePlayers);
    ...
    private void centerAndCountDownEnd(ArrayList<OfflinePlayer> gamePlayers) {
        for(OfflinePlayer player : gamePlayers) {
        ...
        switch (this.state) {
            ...
            case INGAME:
                if(isCountdown) {
                    ...
                    centerAndCountDownEnd(getIngamePlayers());
                }
                Bukkit.getScheduler().cancelTask(this.gameTaskId);
                this.state = GameState.ENDING;
                calculateRankingsAndDisplaySummaries();
                for (OfflinePlayer player : getAllPlayers()) {
                    leavePlayer(player.getPlayer());
                }
                Bukkit.getScheduler().runTaskLater(FootballPlugin.INSTANCE, this::destroy, 5 * 20);
        }
    }
    ...
    public HashMap<Player, Location> playerSpawnMap() {
        ...
        ArrayList<CasualArena.AttribuedPlayerSpawn> blueSpawns = new ArrayList<>();
        ...
        ArrayList<CasualArena.AttribuedPlayerSpawn> redSpawns = new ArrayList<>();
        spawns.forEach((player, spawnId) -> {
        ...
        if(blueSpawns.size() >= redSpawns.size()) {
            for(int a = 0; a < blueSpawns.size(); a++) {
                CasualArena.AttribuedPlayerSpawn bluePlayer = blueSpawns.get(a);
                CasualArena.AttribuedPlayerSpawn redPlayer;
                try {
            ...
            for(int a = 0; a < redSpawns.size(); a++) {
                CasualArena.AttribuedPlayerSpawn redPlayer = redSpawns.get(a);
                CasualArena.AttribuedPlayerSpawn bluePlayer;
                try {
    ...
    private void createScoreboard(Player player) {
        ...
        ScoreboardSign scoreboardSign = new ScoreboardSign(player, "§b§l" + mode.getModeName() + " " + mode.getSubModeName());
        ...
        if(getPlayerTeam(player) != null) {
            ...
            double mmr = FootballPlugin.INSTANCE.getPlayerService().getPlayerData(player).getMmr().get(FootConfig.CURRENT_SEASON).get(mode);
            Rank rank = FootballPlugin.INSTANCE.getRankingService().getRankByMmr(mmr);

            // Player isn't spectator
            switch (this.state) {
                case WAITING:
                    String personalStats = "§fPersonal stats: §a" + player.getName();
                    if(personalStats.length() > 48)
                        personalStats = personalStats.substring(0, 48);
                    scoreboardSign.setLine(1, "§fMap: §a" + this.map.getName());
                    scoreboardSign.setLine(2, "§c");
                    scoreboardSign.setLine(3, personalStats);
                    scoreboardSign.setLine(4, "§f▍ Played games: §a" + Math.round(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.PLAYED))) + " §8(" + Math.round(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.VICTORIES))) + "W/" + Math.round(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.LOSES))) + "L)");
                    scoreboardSign.setLine(5, "§f▍ Goals: §a" + Math.round(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.GOALS))));
                    scoreboardSign.setLine(6, "§f▍ Assists: §a" + Math.round(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.ASSISTS))));
                    scoreboardSign.setLine(7, "§f▍ Current rank: §6" + rank.getColorCode() + rank.getName() + " Div " + rank.getDivisionNumber(mmr));
                    scoreboardSign.setLine(8, "§b");
                    scoreboardSign.setLine(9, "§fStarting in §c--s");
                    break;
                case INGAME:
            ...
            switch (this.state) {
                case STARTING:
                    if(this.startTimer != null)
                        scoreboardSign.setLine(9, "§fStarting in §a" + this.startTimer  + "s");
                    break;
                case INGAME:
                    scoreboardSign.setLine(1, new LanguageManager.Phrase(LanguageKey.BOSS_GOALS)
                            .replaceVar("currentteamcolor", teamQualifier.getColorCode())
    ...
    private int startWaitingTask() {
        return Bukkit.getScheduler().scheduleSyncRepeatingTask(FootballPlugin.INSTANCE, () -> {
            ActionBar bar = new ActionBar(getAllPlayers(), new LanguageManager.Phrase(LanguageKey.ACTIONBAR_WAITING_PLAYERS)
                .replaceVar("current", String.valueOf(getIngamePlayers().size()))
                .replaceVar("max", String.valueOf(this.mode.getNumberOfSpawns() * this.teams.size()))
                .toString());
    ...
    private FootTeam getWinningTeam() {
        FootTeam blue = teams.get(FootTeamQualifier.BLUE);
        FootTeam red = teams.get(FootTeamQualifier.RED);
        if(blue.getGoals() > red.getGoals())
            return blue;
        else if(red.getGoals() > blue.getGoals())
            return red;
        else
            return null;
    }

    private void calculateRankingsAndDisplaySummaries() {

        FootTeam winningTeam = getWinningTeam();

        HashMap<Player, Double> previousMmr = new HashMap<>();
        HashMap<Player, Double> newMmr = new HashMap<>();
        HashMap<Player, Rank> rankHashMap = new HashMap<>();

        for(OfflinePlayer offlinePlayer : getIngamePlayers()) {
            if(offlinePlayer.isOnline()) {
                Player player = (Player) offlinePlayer;

                boolean hasWon = getPlayerTeam(player).equals(winningTeam);
                double mmrGain = FootballPlugin.INSTANCE.getRankingService().calculateMmr(hasWon, playerGoals.get(player.getUniqueId()), playerPasses.get(player.getUniqueId()), totalGameTime);

                FootPlayer footPlayer = FootballPlugin.INSTANCE.getPlayerService().getPlayerData(player);

                previousMmr.put(player, footPlayer.getMmr().get(FootConfig.CURRENT_SEASON).get(mode));
                newMmr.put(player, previousMmr.get(player) + mmrGain);

                rankHashMap.put(player, FootballPlugin.INSTANCE.getRankingService().getRankByMmr(newMmr.get(player)));

                footPlayer.getMmr().get(FootConfig.CURRENT_SEASON).put(mode, newMmr.get(player));

                footPlayer.getCurrentSeasonStats().put(FootStat.getStat(mode, FootStat.PreciseStat.PLAYED), footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.PLAYED)) + 1);
                if(getWinningTeam() != null && hasWon)
                    footPlayer.getCurrentSeasonStats().put(FootStat.getStat(mode, FootStat.PreciseStat.VICTORIES), footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.VICTORIES)) + 1);
                else if(getWinningTeam() != null)
                    footPlayer.getCurrentSeasonStats().put(FootStat.getStat(mode, FootStat.PreciseStat.LOSES), footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.LOSES)) + 1);

                footPlayer.getCurrentSeasonStats().put(FootStat.getStat(mode, FootStat.PreciseStat.GOALS), footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.GOALS)) + playerGoals.get(player.getUniqueId()));
                footPlayer.getCurrentSeasonStats().put(FootStat.getStat(mode, FootStat.PreciseStat.ASSISTS), footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.ASSISTS)) + playerPasses.get(player.getUniqueId()));

            }
        }

        ComparablePlayerList previousRankings = FootballPlugin.INSTANCE.getRankingService().getRankings(mode, false);
        ComparablePlayerList rankings = FootballPlugin.INSTANCE.getRankingService().getRankings(mode, true);

        for(OfflinePlayer offlinePlayer : getIngamePlayers()) {
            if(offlinePlayer.isOnline()) {
                Player player = (Player) offlinePlayer;
                displayEndSummary(player, previousMmr.get(player), newMmr.get(player), rankHashMap.get(player), rankings.getPosition(player), previousRankings.getPosition(player), rankings.size());
            }
        }

    }

    private void displayEndSummary(Player player, double previousMmr, double newMmr, Rank rank, int rankingPosition, int previousPosition, int rankingPlayers) {
        FootPlayer footPlayer = FootballPlugin.INSTANCE.getPlayerService().getPlayerData(player);

        String recordPrase = "";
        if(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.GOALS)) < playerGoals.get(player.getUniqueId()))
            recordPrase = "§e§lNEW PERSONAL BEST! (GOALS)";
        else if(footPlayer.getCurrentSeasonStats().get(FootStat.getStat(mode, FootStat.PreciseStat.ASSISTS)) < playerPasses.get(player.getUniqueId()))
            recordPrase = "§e§lNEW PERSONAL BEST! (ASSISTS)";
        ...
        player.sendMessage("");
        ChatUtils.sendCenteredMessage(player, recordPrase);
        ...
        ChatUtils.sendCenteredMessage(player, "§fCurrent rank: " + rank.getColorCode() + rank.getName() + " Div " + rank.getDivisionNumber(newMmr) + " §8(MMR: " + new DecimalFormat("##.00").format(newMmr) +  "[" + new DecimalFormat("##.00").format(Math.abs(newMmr - previousMmr)) + " " + (newMmr > previousMmr ? "§a↑" : "§c↓") + "§8])");
        ChatUtils.sendCenteredMessage(player, "§fRanking: §a#" + rankingPosition + "/" + rankingPlayers + " §8[" + Math.abs(rankingPosition - previousPosition) + " " + (rankingPosition > previousPosition ? "§c↓" : "§a↑") + "§8])");
        player.sendMessage("§a§m───────────────────────────────────");
    }

    private static class AttribuedPlayerSpawn implements Comparable<CasualArena.AttribuedPlayerSpawn> {
        ...
        @Override
        public int compareTo(CasualArena.AttribuedPlayerSpawn o) {
